...
public:
	...
	DualityTetris(const char* name) : Scene(name), mTimeSinceSpawn(0.0), mLastEmit(false), mViscous(false) {}
			...
			for (int z = 0; z < dimz; ++z)
				for (int x = 0; x < dimx; ++x)
					CreateParticleShape(
						...
						(Vec3(float(x) / float(dimx), float(y) / float(dimy) + 0.5f + float(y) * radius * 0.25f, float(z) / float(dimz)) - 0.5f) * 2.0f,
